// ==UserScript==
// @name         视频旋转（全屏自适应 · 比例裁切 · 一键铺满）
// @namespace    dale.local
// @version      11.3
// @description  YouTube / Bilibili 控制栏放三个小图标（旋转 / 设置 / 全屏）；全屏时按整块屏幕自适应，比例选项改为裁切而非拉伸，另有「一键铺满」；缩放长条**点哪就多大**（一次点到位，也可拖动 / 滚轮 / 快选档位）；全屏时按钮在右下角
// @match        *://*/*
// @grant        none
// @run-at       document-end
// ==/UserScript==
...
(function () {
  ...
  function ensureStyle() {
    ...
    s.textContent =
      ...
      '.dal-drawer .dal-lab{width:30px;opacity:.6;flex:none;}' +
      ...
      'word-break:break-all;border-top:1px solid rgba(255,255,255,.12);padding-top:7px;}';
  ...
  function buildBar() {
    ...
    bRot.title = '旋转 90°（按住 Shift 反向 · Option+R）';
  ...
  function buildDrawer() {
    ...
    // 角度
    var rDeg = chipRow('角度', [0, 90, 180, 270].map(function (d) {
    ...
    // 比例
    var rMode = chipRow('比例', MODES.map(function (m, i) {
    ...
    // 显示范围：自动 / 播放器框 / 整块屏幕
    var SCOPE_LAB = ['自动', '播放器', '整屏'];
    var rScope = chipRow('范围', SCOPE_LAB.map(function (m, i) {
      return { label: m, val: i };
    }), function () { return false; }, function (val) {
      SCOPE = val; save(); enforce(true);
    });
    rScope.title = '自动：全屏时用整块屏幕、窗口时用播放器框。「整屏」可让窗口模式下也铺满屏幕';
    ...
    bMinus.textContent = '−';
    bMinus.title = '缩小（点一下 5%，离 100% 越远步子越大；按住不放连续快调；⌥− 同效）';
    holdRepeat(bMinus, -1);
    ...
    bPlus.textContent = '+';
    bPlus.title = '放大（点一下 5%，离 100% 越远步子越大；按住不放连续快调；⌥= 同效）';
    holdRepeat(bPlus, 1);
    ...
    rng.title = '点长条上任意位置 → 直接就缩放到那么大；按住拖动跟手；' +
    ...
    var rZPre = chipRow('快选', [50, 75, 100, 125, 150, 200, 315, 400].map(function (p) {
    ...
    bZero.textContent = '对中';
    bZero.title = '清掉手动偏移，回到自动居中';
    ...
    bFill.textContent = '一键铺满';
    bFill.title = '自动选好角度并铺满屏幕（缩放 100%、偏移归零）';
  ...
  function updateStatus() {
    ...
    if (isDefault()) {
      drawer._st.textContent = '原画未调整' + (isFullscreenNow() ? '（全屏）' : '');
      return;
    }
    if (!d) { drawer._st.textContent = '…'; return; }
    drawer._st.textContent =
      '视频 ' + arName(d.视频ar) + '（' + MODES[MODE] + ' 框）· ' +
      '范围 ' + d.Dw + '×' + d.Dh + (d.全屏 || d.原生全屏 ? ' 全屏' : ' 窗口') +
      ' · 缩放 ' + Math.round(d.zoom * 100) + '%' +
      (d.裁切 === '无' ? '' : ' · 裁切 ' + d.裁切);
  }
